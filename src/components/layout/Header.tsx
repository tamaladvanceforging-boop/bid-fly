...
import { useAppStore, useThemeStore, useUIStore } from '@/stores/app.store';
import { useAuthStore } from '@/stores/auth.store';
import { formatRelativeTime } from '@/lib/utils';
import { realtimeSync, type NetworkSyncStatus } from '@/lib/utils';
...
export function Header() {
  ...
  function handleSearchSubmit(e: React.FormEvent) {
    ...
    router.push(`/tenders?q=${encodeURIComponent(searchQuery.trim())}` as any);
  }
          ...
          <Popover open={alertsOpen} onOpenChange={setAlertsOpen}>
            ...
            <PopoverContent className="w-80 p-0" align="end">
              ...
              <div className="p-2 border-t text-center">
                <Button variant="ghost" size="sm" className="w-full text-xs" onClick={() => { router.push('/alerts' as any); setAlertsOpen(false); }}>
                  View all alerts
                </Button>
              </div>
            </PopoverContent>
