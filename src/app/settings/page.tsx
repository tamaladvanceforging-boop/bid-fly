...
import {
  ...
  Eye, EyeOff, Copy, Check, Shield, User, Key, Database, FileCode, Globe, Lock, Trash2
} from 'lucide-react'
...
export default function SettingsPage() {
  ...
  const { user, updateProfileAsync, logout } = useAuthStore()
...
`
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      if (window.bidfly?.auth?.changePassword && user) {
        const res = await (window.bidfly.auth.changePassword as any)(user.id, oldPassword, newPassword)
        if (res.success) {
  ...
  const handleClearAllData = () => {
    if (typeof window !== 'undefined' && (window.bidfly as any)?.clearAllData) {
      (window.bidfly as any).clearAllData()
    }
    if (typeof window !== 'undefined') {
      const allKeys = Object.keys(localStorage)
      allKeys.forEach(k => {
        if (k.startsWith('bidfly_')) localStorage.removeItem(k)
      })
    }
    logout()
    addToast({ title: 'Application Reset', description: 'All demo records, user accounts, and database cleared.', variant: 'success' })
    router.push('/login')
  }

  const handleLoadSampleData = () => {
    if ((window.bidfly as any)?.loadSampleData) {
      (window.bidfly as any).loadSampleData()
      addToast({ title: 'Demo Samples Loaded', description: 'Sample tenders, bids, and vendors populated.', variant: 'success' })
      setTimeout(() => {
        window.location.reload()
      }, 500)
    } else {
      addToast({ title: 'Demo loaded', variant: 'default' })
    }
  }
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border bg-muted/20 gap-3">
                <div>
                  <p className="font-semibold text-sm">Clear All Data & Reset Database</p>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Permanently wipe all records, tenders, bids, user accounts, and local storage. Starts a fresh workspace.
                  </p>
                </div>
                <Button variant="destructive" size="sm" onClick={handleClearAllData} className="font-bold text-xs shrink-0">
                  <Trash2 className="h-4 w-4 mr-2" /> Clear All Data & Logout
                </Button>
              </div>

              <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border bg-muted/20 gap-3">
                <div>
                  <p className="font-semibold text-sm">Populate Demonstration Data</p>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Optionally populate sample government & corporate tenders (NTPC, Railways, IOCL), bids, scoring sheets, and vendor profiles.
                  </p>
                </div>
                <Button variant="outline" size="sm" onClick={handleLoadSampleData} className="font-bold text-xs shrink-0">
                  <RotateCcw className="h-4 w-4 mr-2" /> Load Demo Samples
                </Button>
              </div>
              ...
              <div className="flex items-center justify-between p-4 rounded-xl border bg-muted/20">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">BidFly Enterprise Suite 2.0 (Clean Build)</p>
                </div>
                <Badge variant="outline" className="font-mono text-xs">v2.0.0-RELEASE</Badge>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
